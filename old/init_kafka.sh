...
if [ -d "${KAFKA}checkpoint" ]; then
	echo "Suppression de l'ancien dossier ${KAFKA}checkpoint ..."
...
(cat /home/cesar/cours/ensae/donnees_distrib/projet/mobile_data/kafka_ingestion.csv | split -l 30 --filter="$KAFKA/bin/kafka-console-producer.sh --broker-list localhost:9092 --topic antennesInput; sleep 10" > logs/cat.log ) &
sleep 1
